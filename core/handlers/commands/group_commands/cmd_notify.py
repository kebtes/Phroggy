...
async def id_group(message: types.Message, command: CommandObject):
    ...
    if not (
        await groups.is_admin(group_id, user_id)
        or await groups.is_moderator(group_id, user_id)
    ):
    ...
    try:
        ...
        await groups.update_preference(group_id=group_id, notify=auto_delete)
